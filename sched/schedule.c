 ...
 *   do_superman:         Enable superman status
 *   do_kryptonite:       Disable superman status
 */
#include "sched.h"
#include "schedproc.h"
#include <assert.h>
...
PUBLIC int do_noquantum(message *m_ptr)
{
	...
	/* If the process can be lower and it's not superman */
	if (rmp->priority < MIN_USER_Q && rmp->superman != 1) {
...
PUBLIC int do_start_scheduling(message *m_ptr)
{
	...
	case SCHEDULING_INHERIT:
		...
		/* If parent is superman, child should be superman too */
		if (schedproc[parent_nr_n].superman == 1) {
		    rmp->superman = 1;
            printf("log: After fork, the child's priority is:%d\n", rmp->priority);
		} else {
		    rmp->superman = 0;
		}
		break;
